...
function ContactTable({ data, minus, setPagination, total, pagination, handleAdd, filter }) {
    ...
    return (
        <div >
            <ContactModal position={modal.position} display={modal.display} data={contactSelected} handleClose={() => setModal({ ...modal, display: false })} />
            <h5>Contacts</h5>
            <Row>
                <Col md='9'>
            ...
            <Row>
                <Col md={!filterDisplay ? 12 : 8}>
                    ...
                    <BlipTable
                        ...
                        canSelect={true}
                        sort={sort}
                        onItemClick={(event, item) => { handleOpenModal(event, item) }}
                        onSortSet={(item) => { sortData(contacts, item) }}
                        bodyHeight="1300px"
                        selectedItems={selected}
                        actions={[<Button variant="success" style={{ display: 'block', margin: 'auto' }} onClick={() => { handleAdd(selected); setSeleted([]) }}> <FiUserPlus /> Add</Button>]}
                    />
                ...
                <Col md={!filterDisplay ? 0 : 4}>
                    <FilterForm activated={filterDisplay} disable={() => setFilterDisplay(false)} applyFilter={(fill) => { filter(fill); }} />
                </Col>
            </Row>
